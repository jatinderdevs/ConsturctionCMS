const Invoice = require("../models/jobInvoice");
const PDFDocument = require("pdfkit");

module.exports.genrate = async (req, res, next) => {
  const { totalInvoiceAmount, jobId, username } = req.body;
  const invoiceNumber = Math.floor(1000 + Math.random() * 9000);
  const { _id } = req.user;
  console.log(username, _id);
  if (username !== _id.toString()) {
    req.flash("error", "you are not authorized for this request");
  ...
  req.flash("success", "Invoice has been genrated Successfully");
};

module.exports.invoicePDF = async (req, res, next) => {
  const invoiceData = {
    invoiceNumber: "73",
